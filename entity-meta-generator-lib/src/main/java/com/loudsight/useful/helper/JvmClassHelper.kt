...
object JvmClassHelper {
    ...
    private fun getClassName(primitiveName: String): String {
        ...
        if (!primitiveName.contains('.')) {
            className = primitiveName.toLowerCase()
        } else if (className.equals("kotlin.String")) {
    ...
    @JvmStatic
    fun <T> classForName(className: String, classLoader: ClassLoader): Class<T> {
        return if (isPrimitive(className)) {
            getPrimitiveClass(className)
        } else try {
            uncheckedCast<Class<T>>(Class.forName(className, true, classLoader))
        } catch (e: Exception) {
